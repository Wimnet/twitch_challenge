...
function DroppedFramesRule() {
    ...
    function getMaxIndex(rulesContext) {
        ...
        switchRequest.reason = {rule: "DroppedFrameRule"};
            ...
            for (let i = 1; i < dfh.length; i++) { //No point in measuring dropped frames for the zeroeth index.
                if (dfh[i]) {
                    ...
                    if (totalFrames > GOOD_SAMPLE_SIZE && droppedFrames / totalFrames > DROPPED_PERCENTAGE_FORBID) {
                        ...
                        //console.log('DroppedFrameRule', 'index: ' + maxIndex + ' Dropped Frames: ' + droppedFrames + ' Total Frames: ' + totalFrames);

                        break;
                    }
                }
            }
            const otherSwitchRequest = SwitchRequest(context).create(maxIndex, {droppedFrames: droppedFrames});
            otherSwitchRequest.reason = {rule: "DroppedFrameRule"};

            return otherSwitchRequest;
        }
