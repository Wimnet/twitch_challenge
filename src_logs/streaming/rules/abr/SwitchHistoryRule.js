...
function SwitchHistoryRule() {
    ...
    function getMaxIndex(rulesContext) {
        ...
        switchRequest.reason = {rule: "SwitchHistoryRule"};
        ...
        for (let i = 0; i < switchRequests.length; i++) {
            if (switchRequests[i] !== undefined) {
                ...
                if (drops + noDrops >= SAMPLE_SIZE && (drops / noDrops > MAX_SWITCH)) {
                    ...
                    switchRequest.reason = {rule: "SwitchHistoryRule", quality: switchRequest.quality, drops: drops, noDrops: noDrops, dropSize: dropSize};
                    break;
                }
            }
        }
